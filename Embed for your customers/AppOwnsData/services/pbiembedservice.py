...
class PbiEmbedService:
    ...
    def get_embed_params_for_single_report(self, workspace_id, report_id, additional_dataset_id=None, filter_query=None):
        '''Get embed params for a report and a workspace with optional filter

        Args:
            workspace_id (str): Workspace Id
            report_id (str): Report Id
            additional_dataset_id (str, optional): Dataset Id different than the one bound to the report. Defaults to None.
            filter_query (str, optional): OData filter query to apply to the report. Defaults to None.
        ...
        embed_url = api_response['embedUrl']
        
        # Append filter query if provided
        if filter_query:
            embed_url += f'&filter={filter_query}'

        report = ReportConfig(api_response['id'], api_response['name'], embed_url)
